...
function deliveryLabel(m: DeliveryMethod) {
...
export async function POST(req: Request) {
  ...
  const orderNumber = `${datePrefix}${todayCount + 1}`;

  // Создаём заказ и обновляем сток в рамках транзакции, чтобы избежать гонок.
  ...
  return NextResponse.json({ orderId: order.orderNumber });
}
